...
//    Serial.print(newData);
    while (Serial.available() > 0 && newData == false) {
        ...
        if (recvInProgress == true) {
            if (rc != endMarker) {
//                Serial.println("in rc!= endMarker if");
...
{ 
 //*********PITCH SECTION
 //***ANGLE DEFINED
 //*** potentiometer value of the desired position for a given Motor2_Speed
     int desiredpos = 981.00 - 933.00*((Motor2_Speed)/45.00);
...
//***DIRECTION OF PITCH 
     if (Cposition >= desiredpos) // IF Current position is lower than desired position then lin_act needs to extend 
     {
     ...
     else if (Cposition < desiredpos) // IF Current position is higher than desired position then lin_act needs to retract 
     {
