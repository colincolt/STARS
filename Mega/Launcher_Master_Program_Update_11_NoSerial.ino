...
//***FEEDER MOTOR VARIABLES
 ...
 PID myPID_Feeder(&Input_3, &Output_3, &Setpoint_3,0.001,0.0002,0.0002,DIRECT);
...
int drillType;              // Static = 0; Dynamic = 1;
...
unsigned long Base_Drill_Time = 10050; // Base time for a drill (10 seconds)
...
//  digitalWrite(IN2, LOW);
  ...
  myPID_Feeder.SetMode(AUTOMATIC);
  myPID_Feeder.SetSampleTime(100);
  myPID_Feeder.SetOutputLimits(-255,255);
...
//    Serial.println(ball_fed);
//    Serial.print("Setpoint 1 = ");
//    Serial.println(Setpoint1);
//    Serial.print("Setpoint 2 = ");
//    Serial.println(Setpoint2); 
//    Serial.print("Motor 1 Time 1 = ");
//    Serial.println(Motor1_Time1);
//    Serial.print("Motor 1 Time 2 = ");
//    Serial.println(Motor1_Time2);  
//    Serial.print("Motor 2 Time 1 = ");
//    Serial.println(Motor2_Time1);
//    Serial.print("Motor 2 Time 2 = ");
//    Serial.println(Motor2_Time2);  
//    Serial.print("Motor 1 Flag = ");
//    Serial.println(Motor1_Flag);
//    Serial.print("Motor 2 Flag = ");
//    Serial.println(Motor2_Flag);  
...
//    Serial.print("Motor 1 Startup = ");
//    Serial.println(Motor1_Startup);        
//    Serial.print("Motor 2 Startup = ");
//    Serial.println(Motor2_Startup);
//    Serial.println();
...
//    } 
//
//    if (((abs(Input1 - Setpoint1) <= 0.04*Setpoint1) && (abs(Input2 - Setpoint2) <= 0.04*Setpoint2) && (ballFeed == 1) && ((millis() - Motor_Start_Time) >= 3000)) || ((millis() - Motor_Start_Time) >= 10000))
//       {
...
//------------------------- for DC feeder --------------------------//
  ...
  if (ballFeed == 1){
    ...
    if (drillType == 0){
      ...
      if (((abs(Input1 - Setpoint1) <= 0.04*Setpoint1) && (abs(Input2 - Setpoint2) <= 0.04*Setpoint2) && ((millis() - Motor_Start_Time) >= 1000)) || ((millis() - Motor_Start_Time) >= 3000))      
      {
      ...
      if (((millis() - Motor_Start_Time) >= 3000))      
      {
          Launch();
      }
    }
   ...
   if (Process_Stage == 2)
   {
      ...
      if ((targetChoice > 0) && ((difficulty > 0) && (difficulty <= 5))){
        ...
        Serial.println((millis() - Drill_Start_Time));
        if ((millis() - Drill_Start_Time) >= wirelessDelay){
           send(); 
        }
        if ((millis() - Drill_Start_Time) >= (wirelessDelay + 2000)){
...
//        Serial.println(Speed_Poll);
        if (rslt2) {
            if ( radio.isAckPayloadAvailable() ) {
                ...
                Serial.println("Got Target Data");
...
//     Serial.println(EncoderCount);
     ...
     if ((EncoderCount < End_Count)) {
      
         analogWrite(ENA,255);
     }

     else if ((EncoderCount >= End_Count) || (EncoderCount < 0)){
...
{
     EncoderCount = (myEnc.read());// number of countable events seen
//     Serial.print("Feeder Check - Encoder Count = ");
//     Serial.println(EncoderCount);
     
     if ((EncoderCount >= End_Count) && (Dir == 1)){ // (!digitalRead(In1)) && (digitalRead(In2))) { //&& (In1 == LOW) && (In2 == HIGH)){
        analogWrite(ENA,Stop);
//        Serial.println("Check - Stop");
     }

     if ((EncoderCount < 0) && (Dir == 0)){ //(digitalRead(In1)) && (!digitalRead(In2))){  // && (In1 == HIGH) && (In2 == LOW)){
        analogWrite(ENA,Stop);
//        Serial.println("Check - Stop 2");
     }
...
//     Serial.print("EncoderCount (Reset) = ");
//     Serial.println(EncoderCount);
     Input_3 = EncoderCount;    
     Setpoint_3 = 0;
     myPID_Feeder.Compute();
...
//********ROTATION AMOUNT FROM LEFT OF CENTRE POSITION->EncoderCount=0 (COUNTER-CLOCKWISE ROTATION )
        if (Output_3 > 0)
        {
           ...
           Feeder_PWM = map(Output_3, 0, -255, 0 , 255);
        
        }

     if (abs(EncoderCount) < Barrier) {

         analogWrite(ENA,Feeder_PWM);
     }
...
//    Motor2_Speed = atoi(strtokIndx);     // convert this part to an integer
    ...
    strtokIndx = strtok(NULL, ","); // this continues where the previous call left off
    drillType = atoi(strtokIndx);     // convert this part to an integer 
}
...
//    Serial.print("drillType = ");
//    Serial.println(drillType);
}
