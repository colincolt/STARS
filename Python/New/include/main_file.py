                            ...
                            if len(self.distances) == self.max_measures:
                                ...
                                if self.future_flag.is_set():
                                    ...
                                    while looped < 15:
                                        ...
                                        looped += 1
                                    self.future_distance = new_distance
        ...
        if self.StartLauncher:
            try:
                Launch = launcher.Launcher(self.guiData, self.mega_data, self.final_dist_l, self.future_dist_l, working_on_the_Pi, WHITE,
                                           self.kill_event, self.pause_event, self.py_reset_event, self.launch_event, self.voice_control, self.results,self.player_choice, self.launcher_shutdown, self.send_launchdist,self.future_flag)
