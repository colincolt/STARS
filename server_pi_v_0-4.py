        ...
        while not self.py_shutdown_flag.isSet():
            if drillType == "Dynamic":
                ...
                try:
                    ...
                    # Get FINAL_DIST from Launcher(Thread) _______________
                    try:
                    ...
                    # Get FUT_FINAL_DIST _______________
                    try:
                    ...
                    # ****CALCULATE YAW MOTOR SPEED & QUERY TABLE FOR PITCH ANGLE**** _______________***
                    #
                    if futureDist:
                        usedDistance = futureDist
                    else:
                        if finalDist:
                            usedDistance = finalDist
                    ...
                    if usedDistance == futureDist:
                        # << CASE 1: Only have to 'predict'/anticipate future lateral displacement
                        # PITCH ANGLE:
                        pitchAngle = 45 # <<<*************!!!!!!!!! Query table for angle at futureDist or use 'someFactor'

                        # calculate Lateral speed for Yaw motor:
                        PixToDistApprox = usedDistance / 250  # <<< BS Conversion at the moment
                        Distance = latPixelDisp * PixToDistApprox
                        latDispDeque.appendleft(Distance)
                        if len(latDispDeque) > 10:
                            latDispDeque.pop()
                        latSpeed = (latDispDeque[1] - latDispDeque[0]) / latTimeDeque[0]

                        # YAW MOTOR SPEED
                        motorSpeed = latSpeed * someFactor  # <<<*************!!!!!!!!!  Query table or use 'someFactor' yawSpeed based on latSpeed
                        #
                        #
                        #
                        #
                    else:
                        # << CASE 2: Have predict FUT_FINAL_DIST ourselves if we dont get it
                        pyDistDeque.appendleft(usedDistance)
                        if len(pyDistDeque) > 10:
                            pyDistDeque.pop()
                        # calculate future z-distance of the player from available data:
                        if oldzDist is None:
                            oldzDist = stereoDist
                            speed = 0
                        else:
                            tempDist = stereoDist - oldzDist
                            speed = tempDist / latTimeDeque[0]
                            # the idea is to stay 5 seconds ahead of the player
                            futureDistCalc = float(stereoDist) + (speed * 5) # 5 seconds

                            # PITCH ANGLE:
                            pitchAngle = 45  # <<<*************!!!!!!!!!  Query table for angle at futureDistCalc or use 'someFactor'
                        ...
                        if len(latDispDeque) > 10:
                            latDispDeque.pop()
                        latSpeed = latDispDeque[1] - latDispDeque[0]
                    ...
                    # if stereoDistance <= 10:
                    #     pitchAngle = 25  # 10 DEGREES
                    # if 10 < stereoDistance <= 45:
                    #     pitchAngle = 45  # 20 DEGREES
                    # ##                if 10 < stereoDistance <= 15:
                    # ##                    pitchAngle = 30  # # 30 DEGREES
                    # ##                if 15 < stereoDistance <= 20:
                    # ##                    pitchAngle = 40  # # 30 DEGREES
                    # ##                if 20 < stereoDistance <= 50:
