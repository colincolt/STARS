    ...
    def run(self):
        ...
        while True:
            try:
                ...
                speed = self.getguiStack.speed.peek()
                difficulty = self.getguiStack.difficulty.peek()
                drillType = self.getguiStack.drilltype.peek()
                lock.release()
...
def StereoscopicsThread(StereoOutput, stereoStack):
...
class Launcher(Thread):
    def __init__(self, sendVoiceCommandStack, sendDistanceStack):  # , guiStack):
        Thread.__init__(self)
...
def EvoRanger():
    ...
    while a == True:
        try:
            evo = serial.Serial("/dev/ttyACM0", baudrate=115200,
                                timeout=2)  ##MANUALY INPUT the 'portname' /tty/.../
...
def startMainFile(speed, difficulty, drilltype):  # , args): ## NOT A THREAD, performs the bulk of calculation
    ...
    try:
        process = Thread(target=StereoscopicsThread, args=[StereoOutput, stereoStack])
        process.start()
    except Exception as e:
    ...
    # _ Get and wait for the stereo distance
    stereoDistance = stereoStack.distance.peek()
    while stereoDistance == None:
            print("Main File waiting for stereoStack")
            stereoDistance = stereoStack.distance.peek()
            time.sleep(0.25)

    EvoDistance = EvoRanger()
