...
class PitchYaw(Thread):
    def __init__(self, getstereoStack, getguiStack, finalDistStack):  # getdistanceStack
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                while getdata = False:
                    if MEGA.is_open:
                        ...
                        while not dataPresent:
                            size = MEGA.inWaiting()
                            if size:
                                megaDataStr = MEGA.readline()  # READ DATA FORMATTED AS ('< 1 2 3 4 5 >')
                    else:
                ...
                if FINAL_DIST < 5:
                    motorSpeed1, motorSpeed2 = 1000
                elif 5 <= FINAL_DIST < 10:
                    motorSpeed1, motorSpeed2 = 1500
                elif 10 <= FINAL_DIST < 20:                
                    motorSpeed1, motorSpeed2 = 2000
                elif 20 <= FINAL_DIST < 25                
                    motorSpeed1, motorSpeed2 = 2500
                else:
                    print('out of range')
...
def startMainFile(speed, difficulty, drilltype):  # , args): ## NOT A THREAD, performs the bulk of calculation
    ...
    time.sleep(5)
    try:
        pitchYawthread = PitchYaw(stereoStack, guiStack, finalDistStack)   # distanceStack
        pitchYawthread.start()
    except Exception as e:
        print('Pitch and Yaw thread didnt start because of exception ' + e)
    try:
        startLauncherThread = Launcher(voiceCommandStack, distanceStack, guiStack, finalDistStack)
    ...
    while True:  # << FINAL DISTANCE CALCULATIONS
        ...
        if isinstance(finalDist, float):
            finalDist = stereoDist
        else:
            continue  # << Goes back to top of while loop
        
        if isinstance(lidarDist, float):
            if stereoDist - lidarDist > 5:
                total = total - 1
            else:
                finalDist += lidarDist
        else:
        ...
        if isinstance(evoDist2, float)
            if stereoDist - evoDist2 > 5:
                total = total - 1
            else:
                finalDist += evoDist2
        else:
