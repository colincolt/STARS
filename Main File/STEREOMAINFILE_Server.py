...
class Pitch_Yaw():
    def __init__(self, data, res_q):
      threading.Thread.__init__(self)
      self.stereoObject = stereoClass(___, ___, ___)
      self.req_q = data
      self.res_q = res_q

    def run(self):
        data = data_class_res.get()
        if data != 0:
            print(data.distance)
...
##MAIN CODE##
#Start Thread
threading.Thread(name = "Pitch_Yaw", target=Pitch_Yaw, args=(data_class_res)).start()
